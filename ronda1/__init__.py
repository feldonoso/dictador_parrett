...
def get_task_module(player):
    ...
    """
    from . import task_decoding, task_matrix, task_transcription
...
class ResultsA(Page):
    ...
    def vars_for_template(player: Player):
        ...
        group = player.group
        return dict(offer=player_a.premio - group.transferencia)
    ...
    def before_next_page(player: Player, timeout_happened):
        ...
        group = player.group
        player_a = player.group.get_player_by_id(1)
        convert_a = float(player_a.premio - group.transferencia)
        participant.pago_a = format(float(4 + convert_a/10),'.2f')
...
class ResultsB(Page):
    ...
    def before_next_page(player: Player, timeout_happened):
        participant = player.participant
        group = player.group
        convert_b = float(group.transferencia)
        participant.pago_b = format(float(4 + convert_b/10),'.2f')
...
class Choice(Page):
    ...
    def vars_for_template(player):
        ...
        print("myList", myList)
        print("ID Jugador actual", player.id_in_subsession)
        print("ID compañero B", player_b.id_in_subsession)
        print("Correctas jugador A", player.num_correct)
        print("Correctas jugador B", player_b.num_correct)
        ...
        if (player_b.num_correct>mediana):
            print("player_b.num_correct>mediana")
            ...
            print("player_b.num_correct==mediana)and(player.tiebreaker==3")
            if((player.tiebreaker==3)&(lista[0]==0)):
                print("player.tiebreaker==3)&(lista[0]==0")
                player.mensaje="su <strong>Jugador B</strong> es parte del 50% <strong>superior</strong>, por lo que a usted se le ha asignado el premio de"
                player.monto_generado=150
                player.tiebreaker=1
                player.premio=150
                ...
                print("player.tiebreaker==3)&(lista[0]==1")
